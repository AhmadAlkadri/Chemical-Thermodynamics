...
def bubble_temperature(
    ...
    def get_error(T_test, use_eos=False):
        # Return log(sum(yi))
        if T_test <= 0:
            return -100.0  # Should not happen with clamp
                ...
                if activity_model:
                    gamma_l = _as_array(
                        activity_model.activity_coefficients(
                            mixture=mixture, temperature_K=T_test, composition=x.tolist()
                        )
                    )
        ...
        if use_eos:
            ...
            try:
                ...
                if activity_model:
                    gamma_l = _as_array(
                        activity_model.activity_coefficients(
                            mixture=mixture, temperature_K=T_test, composition=x_norm.tolist()
                        )
                    )
    ...
    if eos:
        try:
            ...
            if activity_model:
                gamma_l = _as_array(
                    activity_model.activity_coefficients(
                        mixture=mixture, temperature_K=T, composition=x.tolist()
                    )
                )
    ...
    if eos:
        try:
            ...
            if activity_model:
                gamma_l = _as_array(
                    activity_model.activity_coefficients(
                        mixture=mixture, temperature_K=T, composition=x_norm.tolist()
                    )
                )
