...
- **Slice 1: `vlle-plugin-boundary-disposition` (ADR only, then whatever it decides)**
  - `chemthermo.vlle` is a public plugin boundary (`get_vlle_engine`, `VLLEEngine`, `VLLEResult`, ADR-0001) added before any of this existed, and `flash_tp(flash_mode="vlle")` still raises a `ModelError` telling the caller to install an out-of-tree plugin - for a capability that is now in-tree (ADR-0011). That is now misleading, and it is a **public API** question, so it gets its own ADR before any code moves.
  - The question to answer: does the boundary become the way to reach a *different* engine (a user's own multiphase solver, PC-SAFT-backed VLLE), or is it deprecated and removed? Either way, decide the compatibility story for `chemthermo.vlle`'s exported names and for `flash_mode="vlle"`'s error.
  - Do not change `chemthermo.vlle`, `tests/test_vlle_plugin.py` or the `"vlle"` branch of `flash_tp` before that ADR exists.
- **Slice 2: PC-SAFT (Stage F)**
