...
- **Slice 1: `flash-phase-addition`**
  - Capability: multiphase flash (multiphase Rachford-Rice / Michelsen multiphase split) so `FlashResult` can carry more than two phases and the post-split failure of ADR-0009 becomes an answer rather than a `ConvergenceError`.
  - Requirements: extend the `FlashResult` phase-naming contract beyond `liquid`/`vapor`/`liquid1`/`liquid2`, keep the two-phase results of `flash-auto-phase-detection` and `flash-lle-activity` unchanged where no third phase exists, and give the vapor/liquid label a basis better than volatility ordering (ADR-0008 decision 3) if more than two phases can appear.
- **Slice 2: VLLE**
