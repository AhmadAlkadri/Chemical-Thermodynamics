...
- **Slice 1: `pcsaft-association` (recommended next)**
  ...
  - Also queued, in this order after association: **`flash-phase-labels-by-density`** (ADR-0015 decision 2, ADR-0016 "Next slice") - name a phi-phi result's phases from the density root each was evaluated on, and carry that density into `FlashResult`, instead of naming them by a Wilson volatility ranking; it is the last user-visible phi-phi field that is a convention rather than a measurement, and it is the prerequisite for reporting phase densities at all. Then **`flash-phase-addition-eos`**, for which ADR-0016 supplied the missing inner mechanism (an `NP = 2` solve that treats a fraction outside `[0, 1]` as information) but which still needs a real three-phase EOS state to exercise it.
